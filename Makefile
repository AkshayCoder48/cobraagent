...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app cobraagent db upgrade
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		cobraagent user list 2>/dev/null \
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app cobraagent db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app cobraagent db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run cobraagent db migrate -m "initial" || true
	cd backend && uv run cobraagent db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend cobraagent db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend cobraagent db upgrade

db-downgrade:
	uv run --directory backend cobraagent db downgrade

db-current:
	uv run --directory backend cobraagent db current

db-history:
	uv run --directory backend cobraagent db history

# === Server ===
run:
	uv run --directory backend cobraagent server run --reload

run-prod:
	uv run --directory backend cobraagent server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend cobraagent server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend cobraagent user create-admin

user-create:
	uv run --directory backend cobraagent user create

user-list:
	uv run --directory backend cobraagent user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/cobraagent"
...
help:
	@echo ""
	@echo "CobraAgent - Available Commands"
	@echo "======================================"
	@echo ""
