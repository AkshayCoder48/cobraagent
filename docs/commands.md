...
```bash
cd backend
uv run cobraagent <group> <command> [options]
```

### Server Commands

```bash
uv run cobraagent server run              # Start dev server
uv run cobraagent server run --reload     # With hot reload
uv run cobraagent server run --port 9000  # Custom port
uv run cobraagent server routes           # Show all registered routes
```
...
```bash
uv run cobraagent db init                  # Run all migrations
uv run cobraagent db migrate -m "message"  # Create new migration
uv run cobraagent db upgrade               # Apply pending migrations
uv run cobraagent db upgrade --revision e3f  # Upgrade to specific revision
uv run cobraagent db downgrade             # Rollback last migration
uv run cobraagent db downgrade --revision base  # Rollback to start
uv run cobraagent db current               # Show current revision
uv run cobraagent db history               # Show migration history
```
...
uv run cobraagent user create

# Create user non-interactively
uv run cobraagent user create --email user@example.com --password secret

# Create user with specific role
uv run cobraagent user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run cobraagent user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run cobraagent user create-admin --email admin@example.com --password secret

# Change user role
uv run cobraagent user set-role user@example.com --role admin

# List all users
uv run cobraagent user list
```
...
```bash
uv run cobraagent cmd <command-name> [options]
```
...
```bash
uv run cobraagent cmd my-command --name test
```
